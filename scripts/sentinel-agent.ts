...
/**
 * Zipd Anomaly Detection Agent — Sentinel
 * ═══════════════════════════════════════════════
 ...
 *
 * ANTIGRAVITY UPDATE:
 * Now integrated with the Hermes Telegram Gateway for real-time mobile alerts.
 */
...
function notifyCommander(alert: SentinelAlert) {
  if (alert.severity === 'high' || alert.severity === 'critical') {
    const icon = alert.severity === 'critical' ? '🔴' : '🟠';
    const msg = `${icon} [ZIPD SENTINEL] ${alert.type.toUpperCase()}: ${alert.message}`;
    
    try {
      execSync(`"${HERMES_PATH}" say "${msg}"`, { stdio: 'ignore' });
    } catch (e) {
      console.error('Failed to notify commander via Hermes:', e);
    }
  }
}
...
async function detectVelocitySpikes(sql: SqlClient): Promise<SentinelAlert[]> {
  ...
  const results = await sql`
    ...
    FROM clicks
    WHERE timestamp > NOW() - ((${VELOCITY_WINDOW_MIN} || ' minutes')::interval)
...
async function detectLinkAbuse(sql: SqlClient): Promise<SentinelAlert[]> {
  ...
  const results = await sql`
    ...
    FROM clicks c
    JOIN links l ON c.link_id::text = l.id::text
...
async function main() {
  console.log('');
  console.log('╔══════════════════════════════════════════════════════╗');
  console.log('║           🛡️  ZIPD SENTINEL — Anomaly Agent          ║');
