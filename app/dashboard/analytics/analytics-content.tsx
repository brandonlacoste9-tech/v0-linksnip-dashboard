...
import { Activity, Globe, ArrowLeft, TrendingUp, Users, MousePointerClick, BarChart3, Database, Download, Loader2 } from "lucide-react"
...
export default function AnalyticsContent() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        toast.error("Failed to load Real-Time Status Engine for the Zipd Security")
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleExportClicks = async () => {
    try {
      ...
      if (clicks.length === 0) {
        toast.error("No click data available for export")
        return
      }
      ...
      const csvContent = [
        headers.join(","),
        ...clicks.map(c => `"${(c.timestamp instanceof Date ? c.timestamp : new Date(c.timestamp)).toISOString()}","${c.shortCode}","${c.originalUrl}","${c.ipAddress || ''}","${c.country || ''}","${c.referrer || ''}","${c.userAgent?.replace(/"/g, '""') || ''}"`)
      ].join("\n")
      ...
      anchor.setAttribute("download", `zipd-export-${new Date().toISOString().split('T')[0]}.csv`)
      document.body.appendChild(anchor)
      anchor.click()
      document.body.removeChild(anchor)
      toast.success("Infrastructure analytics exported successfully")
    } catch (error) {
      ...
      <header className="h-20 border-b border-white/5 bg-[#0d1221]/80 backdrop-blur-xl flex items-center justify-between px-8 shrink-0 z-20 shadow-imperial">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-[10px] text-zinc-500 uppercase tracking-[0.2em] font-medium mt-0.5">Live Click Infrastructure</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <Button 
            ...
            disabled={exporting || displayData.totals.totalClicks === 0}
          ...
          <div className="flex items-center gap-2 px-4 py-2 rounded-xl border bg-emerald-500/10 border-emerald-500/20 shadow-[0_0_15px_rgba(16,185,129,0.1)]">
      ...
      <div className="flex-1 overflow-y-auto p-8 scrollbar-imperial" style={{ scrollbarWidth: "thin" }}>
        <div className="max-w-7xl mx-auto space-y-8 animate-float">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
            <StatCard icon={<MousePointerClick className="w-5 h-5" />} title="Aggregate Clicks" value={displayData.totals.totalClicks.toLocaleString()} loading={loading} />
            <StatCard icon={<Users className="w-5 h-5" />} title="Unique Identities" value={displayData.totals.uniqueVisitors.toLocaleString()} loading={loading} />
            <StatCard icon={<TrendingUp className="w-5 h-5" />} title="System Integrity" value="100%" loading={loading} />
          </div>
              ...
              {loading ? (
                ...
                <div className="h-[320px] flex flex-col items-center justify-center text-zinc-600 gap-6">
                  ...
                  <div className="text-center space-y-2">
                    ...
                    <p className="text-xs text-zinc-600 max-w-[280px]">Your infrastructure is ready. Data will appear as links are accessed.</p>
                  </div>
                </div>
              )}
            </CardContent>
