// AccessTreeHUD.tsx
// Spatial gold-filament node graph for access chain visualization
// Displays Origin → Access Keys → Sub-Keys with animated severance on revocation

import { motion, AnimatePresence } from "framer-motion";
import { useState, useEffect, useCallback } from "react";
import { AccessAnchor, AccessKey, AuthorityDelegationEngine } from "@/lib/security/trust/AuthorityDelegationEngine";
...
interface AccessTreeHUDProps {
  engine: AuthorityDelegationEngine;
  originHash: string;
  onRevoke: (targetHash: string) => Promise<void>;
  onEmitInvite: () => Promise<void>;
}

export const AccessTreeHUD = ({ engine, originHash, onRevoke, onEmitInvite }: AccessTreeHUDProps) => {
  const [treeData, setTreeData] = useState<TreeNode | null>(null);
  const [selectedNode, setSelectedNode] = useState<string | null>(null);
  const [isRevoking, setIsRevoking] = useState(false);
  const [purgingNodes, setPurgingNodes] = useState<Set<string>>(new Set());
  const [hoveredNode, setHoveredNode] = useState<string | null>(null);
  const [cohortStats, setCohortStats] = useState({ totalOrigins: 0, totalAccessKeys: 0, totalSubKeys: 0 });

  const buildTreeLayout = useCallback(async () => {
    const origin = await engine.getAccessTree(originHash);
    if (!origin) return null;

    const layoutNode = async (anchor: AccessAnchor, authorityLevel: number, parentX: number, parentY: number, index: number, totalSiblings: number): Promise<TreeNode> => {
      const angle = (index / totalSiblings) * Math.PI * 2;
      const radius = authorityLevel === 0 ? 0 : authorityLevel === 1 ? 180 : 120;
      const x = parentX + Math.cos(angle) * radius;
      const y = parentY + Math.sin(angle) * radius;

      const childrenPromises = anchor.delegatedKeys.map(async (key, i) => {
        const childAnchor = await engine.getAccessTree(key.id);
        if (childAnchor) {
          return await layoutNode(childAnchor, authorityLevel + 1, x, y, i, anchor.delegatedKeys.length);
        }
        return {
          id: key.id,
          label: 'VACANT',
      ...
      const maxDelegations = anchor.maxDelegations || (authorityLevel === 0 ? 5 : authorityLevel === 1 ? 3 : 0);
      ...
      return {
        ...
        x,
        y,
        parentId: anchor.parentIdentityHash,
      };
    };
  ...
  const getNodeLabel = (anchor: AccessAnchor): string => {
    switch (anchor.authorityLevel) {
      case 0: return 'ORIGIN';
      case 1: return 'ACCESS KEY';
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="font-mono text-[10px] tracking-wider text-gold-500/30 mt-1">
            {cohortStats.totalOrigins} SECURITY KEYS · {cohortStats.totalAccessKeys} DELEGATES
          </p>
        </div>
...
const renderNodes = (
  ...
  const traverse = (n: TreeNode) => {
    ...
    const isOrigin = n.authorityLevel === 0;
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2 mb-4">
        ...
        <DetailRow label="Authority Level" value={`Level ${node.authorityLevel}`} />
