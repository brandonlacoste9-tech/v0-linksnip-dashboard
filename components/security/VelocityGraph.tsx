// VelocityGraph.tsx
// Real-time gold-line temporal graph for the Security Console HUD

import { motion } from "framer-motion";
import { useEffect, useState } from "react";
import { AggregationEngine, VelocityMetric, ThreatSurfaceReport } from "@/lib/security/intelligence/AggregationEngine";
...
export const VelocityGraph = ({ engine, refreshIntervalMs = 5000 }: VelocityGraphProps) => {
  ...
  const maxRequests = Math.max(...historicalData.map(d => d.totalRequests), 1);
  const sparklinePoints = historicalData.map((d, i) => {
    const x = (i / Math.max(historicalData.length - 1, 1)) * 100;
    const y = 100 - (d.totalRequests / maxRequests) * 100;
    return `${x},${y}`;
  }).join(' ');
    ...
    <div className="relative bg-navy-950/80 backdrop-blur-xl border border-gold-500/10 rounded-2xl p-8 shadow-imperial">
      ...
      {currentVelocity && (
        <div className="grid grid-cols-4 gap-4">
          <MetricTile label="Total Requests" value={currentVelocity.totalRequests.toLocaleString()} />
          ...
          <MetricTile 
            label="Blocked" 
            value={`${(currentVelocity.blockedRate * 100).toFixed(1)}%`}
            isAlert={currentVelocity.blockedRate > 0.05}
          />
        </div>
      )}
